...
- [x] T036 [US3] Add alerting to `infra/main.bicep`: action group with email + SMS receivers (params `alertEmail`, `smsCountryCode`, `smsPhone`, supplied at deploy time only); scheduled query alert rule (30-min evaluation) over `ContainerAppConsoleLogs_CL` with missed-deadline semantics — fires only when local time in `tz` is past the delivery deadline and no `RUN_SUCCESS digest_date=<today's local date>` exists, DST-correct, self-clearing (contracts/deployment.md resource inventory; ⚠️ marker format is a coupled contract with `main.py` — see contracts/runtime-config.md) — depends on T022 _(2026-06-13: authored + validated locally — `az bicep build`/`lint` clean; `@secure()` no-default alertEmail/smsCountryCode/smsPhone + new `deliveryDeadlineHourLocal` param (deployment.md table updated); query built via `join()` because Bicep `'''…'''` literals don't interpolate; `skipQueryValidation` so a fresh-workspace deploy doesn't fail on the not-yet-existing `ContainerAppConsoleLogs_CL`. Deploy validation = T037, needs Azure)_
...
- [x] T041 [P] Write failing tests in `tests/test_store.py`: retention purge deletes postings (and their `applications` rows, same transaction) with status `new`/`dismissed`/`duplicate` and `fetched_at` older than `JOBAGENT_RETENTION_DAYS` (default 60); any other status is never purged (FR-015, data-model.md retention rules)
- [x] T042 Implement the retention purge in `src/job_agent/store.py` and invoke it as a pipeline stage in `main.py` (purge stage logged with counts) — makes T041 green _(2026-06-17: `store.purge_old_postings()` (inner-join on the three purgeable statuses, single transaction, strict `fetched_at < cutoff`); invoked in `main.py` as a post-send, non-fatal housekeeping stage that always logs the `purge:` line. 132 tests green.)_
- [x] T043 [P] Add `Microsoft.Consumption/budgets` to `infra/main.bicep`: `budgetAmount` param (default 50), alert thresholds at 50% and 80% notifying the action-group/alert email (FR-014; the Anthropic-side console notification is the documented manual step in quickstart.md Bootstrap §6) — depends on T036 _(2026-06-17: RG-scoped `Microsoft.Consumption/budgets@2023-05-01`, 50%/80% Actual thresholds → existing `actionGroup` (email+SMS) + `contactEmails:[alertEmail]` to clear the Bicep type's contactEmails-required quirk. New params `budgetAmount` (=50, now in bicepparam) + `budgetStartDate` (defaults to first of current UTC month via `utcNow()` to dodge the Consumption first-of-month rule). validate-infra.sh exit 0; bicep build clean. ⚠️ startDate validity is enforced at DEPLOY, not compile — verify at T046.)_
- [x] T044 Audit per-run log output across `main.py`, `src/job_agent/fetch.py`, `src/job_agent/score.py`, `src/job_agent/digest.py` against the contracts/runtime-config.md minimum-content rules: stage reached, per-source outcomes with error text, fetched/scored/queued counts, fatal failure detail (stage, exception, config key/resource) — and never secret values, runtime-file contents, or scoring rationale (FR-007, SC-005) _(2026-06-17: PASS, no code changes. Every stage emits an identifiable marker (incl. the new `purge:` line); per-source failures `! {vendor}/{slug} failed: {e}`; counts in one `SCORE_SUMMARY fetched=/scored=/remaining=`; fatal exits name the config key/resource + exception; secrets/file-contents/rationale never printed. Purge-failure line `purge: skipped due to error: {e}` is already a distinct assertable marker. No Python touched — 132 tests unchanged. Findings: docs/work/release-tail-1.0/t044-log-audit.md.)_
