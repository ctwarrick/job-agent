...
def _migrate_fingerprints(conn) -> None:
    ...
    for row in rows:
        new_fp = Posting(
            source="", company=row["company"], external_id="",
            title=row["title"], location=row["location"],
            description=row["description"] or "", url="", posted_at=None,
        ).fingerprint
...
def upsert_postings(postings: Iterable[Posting], path: str | None = None) -> int:
...
def digest_date(tz: str | None = None) -> str:
    """Today's local date (YYYY-MM-DD) in JOBAGENT_TZ (default America/Los_Angeles).
...
def finish_run(
    ...
    *,
    ...
    """Mark a run row finished with its outcome (success|degraded|failed).
    ...
    sql = (
        "UPDATE runs SET finished_at=?, outcome=?, failed_sources=?, detail=? "
        "WHERE id=?"
    )
...
def startup_decision(digest_date: str, force: bool, path: str | None = None) -> str:
    """Evaluate the two-rule startup check (data-model.md "Startup check").

    Returns one of:
      - "skip_inflight":  another execution for this digest_date is running
                          (NULL-outcome row within INFLIGHT_TIMEOUT_SECONDS);
                          not bypassed by JOBAGENT_FORCE.
      - "skip_succeeded": this digest_date already has a success/degraded
                          row; bypassed by force=True.
      - "proceed":        no blocking row, or a stale in-flight row was just
                          marked failed.
    """
    with connect(path) as conn:
        rows = conn.execute(
